AddCSLuaFile()
-- copy because spy made his physical bullet stuff impossible to enable without unpacking the addon. wtf?
CustomizableWeaponry.physicalBulletsEnabled = GetConVar("cw_fas2_physical_bullets"):GetBool()
CustomizableWeaponry.MUZZLE_VELOCITY_MULTIPLIER = GetConVar("cw_fas2_physical_bullet_muzzle_velocity_mult"):GetFloat()
...
CustomizableWeaponry.callbacks:addNew("suppressDefaultBullet", "CW20_suppressDefaultBullet", function(self, startPos, direction, commmandNumber)
    ...
    local struct, index = CustomizableWeaponry:firePhysicalBullet(self:GetOwner(), self:GetClass(), startPos, direction, (self.MuzzleVelocityConverted or 10000), true, self.Damage, nil, commmandNumber)
    if CustomizableWeaponry:processPhysicalBullet(struct, FrameTime()) then
        CustomizableWeaponry:removePhysicalBullet(self:GetOwner(), index)
    end
...
function CustomizableWeaponry:firePhysicalBullet(ply, wepClass, startPos, direction, velocity, isTracer, damage, fallSpeed, bulletID, noRicochet, skipNetwork)
    ...
    if IsValid(wep) then
        ...
        if wep.dt.State ~= CW_AIMING then
            startPos = startPos + ply:EyeAngles():Right() * 1.5
        end
    end
    velocity = velocity * GetConVar("cw_fas2_physical_bullet_muzzle_velocity_mult"):GetFloat()
    ...
    if not skipNetwork and SERVER then
...
function CustomizableWeaponry:processPhysicalBullet(bullet, dt)
    ...
    if trace.Hit or trace.HitSky then
        ...
        if not bullet.noRicochet and not trace.HitSky then
